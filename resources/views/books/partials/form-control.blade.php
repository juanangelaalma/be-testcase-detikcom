<form action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data">
  ...
  <div class="sm:px-0 w-full flex flex-col md:flex-row md:flex-wrap">
      <div class="mb-2 md:w-1/2 md:px-2">
          ...
          <x-text-input id="title" value="{{ old('title') ? old('title') : $book->title }}" class="block mt-1 w-full" type="text" name="title"
      ...
      <div class="mb-2 md:w-1/2 md:px-2">
          ...
          <x-text-input id="quantity" value="{{ old('quantity') ? old('quantity') : $book->quantity }}" class="block mt-1 w-full" type="number" name="quantity"
              required autocomplete="current-quantity" />
          <x-input-error :messages="$errors->get('quantity')" class="mt-2" />
          <div class="mb-2 w-full mt-2">
              <x-input-label for="cover" :value="__('Cover')" />
              <x-file-input id="cover" accept="image/jpeg,image/jpg,image/gif,image/png" value="{{ old('cover') ? old('title') : $book->title }}" name="cover"></x-file-input>
              <x-input-error :messages="$errors->get('cover')" class="mt-2" />
          </div>
      </div>
      <div class="mb-2 md:w-1/2 md:px-2">
          <x-input-label for="file" :value="__('File')" />
          <x-file-input id="file" accept="application/pdf" value="{{ old('file') ? old('file') : $book->file }}" name="file"></x-file-input>
